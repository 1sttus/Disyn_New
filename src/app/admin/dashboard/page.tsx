  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      ...
      if (res.ok) {
        ...
        loadData(); // Reload analytics for conversion shifts if status transitions
      } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { name: "Total Visits", value: analytics.visits, icon: Eye, color: "text-accent-cyan", bg: "bg-accent-cyan/5" },
          { name: "Package Clicks", value: analytics.clicks, icon: BarChart, color: "text-accent-purple", bg: "bg-accent-purple/5" },
          { name: "CRM Leads", value: analytics.conversions, icon: Users, color: "text-accent-green", bg: "bg-accent-green/5" },
          { name: "Conversion Rate", value: analytics.conversionRate, icon: CheckCircle2, color: "text-accent-cyan", bg: "bg-accent-cyan/5" },
        ].map((stat, idx) => (
          <div key={idx} className="glass-panel p-6 border-white/5 bg-[#0D111F]/50">
            ...
            <p className="text-3xl font-black font-outfit text-white mt-4 tracking-tight">
              {stat.value}
            </p>
          </div>
        ))}
      ...
      <div className="glass-panel border-white/5 bg-[#0D111F]/50 overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          ...
          <button
            onClick={loadData}
            className="text-xs font-bold text-accent-cyan hover:text-white transition-colors"
          >
            Refresh Logs
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-500">No leads registered.</div>
        ) : (
                ...
                {leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="p-6">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">{lead.email || "No Email Provided"}</div>
                      <a
                      ...
                      {editingLeadId === lead.id ? (
                        <div className="flex gap-2 items-center">
                          ...
                          <button
                            ...
                            className="text-xs bg-accent-cyan text-primary-bg px-2 py-1 rounded font-bold"
                          >
                            Save
                          </button>
                          <button
                        ...
                        <div className="flex items-center justify-between gap-2 group">
                          ...
                          <button
                            ...
                            className="p-1 rounded hover:bg-white/5 text-gray-500 hover:text-accent-cyan opacity-0 group-hover:opacity-100 transition-opacity"
                            title="Edit Notes"
                          >
